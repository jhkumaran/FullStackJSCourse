...
export const getFilePath = async (
     ...
     filePath =
          height === '' && width === ''
               ? path.resolve(imagesFullPath, `${filename}.jpg`)
               : path.resolve(
                      imagesThumbPath,
...
export const checkThumbAvailable = async (
     filename: string = '',
     height: string = '',
     width: string = ''
): Promise<boolean> => {
...
export const createThumbFile = async (
     filename: string = '',
     height: number = 0,
     width: number = 0
): Promise<string> => {
     const fullPath: string = path.resolve(imagesFullPath, `${filename}.jpg`);
